...
# Chart.r
# Jay Walters, 3 November 2014
# R scripts for creating charts

# Plotting library
library(ggplot2)

# Used to plot distributions of the prior, view and posterior overlaid onto
# a single chart
# Parameters
#   vno - index of view to plot
#   pir - prior estimate of the mean return
#   er - posterior estimate of the mean return
#   Q - estimate of the returns to the views
#   P - pick matrix of asset weights in the views
#   tau - uncertainty in prior estimate
#   Sigma - covariance of returns about unknown mean
#   Omega - matrix of uncertainty in the views
#   postSigma - Covariance matrix of returns about posterior estimate
#
plot_views <- function(vno, pi, per, mQ, mP, pTau, mSigma, mOmega, mPostSigma) {

  # setup results
  results <- list()
  
  # Compute prior mean, view mean and posterior mean
  prm <- mP %*% pi
  vm <- mQ
  pom <- mP %*% per
  
  # Compute prior sigma, view sigma and posterior sigma
  prs <- sqrt(pTau * diag(mP %*% mSigma %*% t(mP)))
  vs <- sqrt(diag(mOmega))
  pos <- sqrt(diag(mP %*% (mPostSigma - mSigma) %*% t(mP)))
  
  # Compute min and max for 3 sigma from each mean
  mn_prm <- round((prm -3 * prs), 2)
  mx_prm <- round(prm + 3 * prs, 2)
  mn_vm <- round((vm - 3 * vs), 2)
  mx_vm <- round((vm + 3 * vs) , 2)
  mn_pom <- round((pom - 3* pos), 2)
  mx_pom <- round((pom + 3 * pos), 2)
  # Compute the min and max for each view (Need to make this a loop on size)
  vmin <- c(min(mn_prm[1], mn_vm[1], mn_pom[1]), min(mn_prm[2], mn_vm[2], mn_pom[2]), min(mn_prm[3], mn_vm[3], mn_pom[3]))
  vmax <- c(max(mx_prm[1], mx_vm[1], mx_pom[1]), max(mx_prm[2], mx_vm[2], mx_pom[2]), max(mx_prm[3], mx_vm[3], mx_pom[3]))
  
  # Now find min and max values for plots
  start <- round(min(prm - 3 * prs),2)
  end <- round(max(prm + 3 * prs),2)
  
  # create our sequence of x values for the plot
  x <- seq(start, end, 0.002)
  n <- NROW(x)
  
  # create prior, conditional and posterior lists to collect the pdf data
  wpr <- list()
  wv <- list()
  wpo <- list()
  for (i in 1:nrow(P)) {
    wpr[[i]] <- dnorm(x, prm[i], prs[i])
    wv[[i]] <- dnorm(x, vm[i], vs[i])
    wpo[[i]] <- dnorm(x, pom[i], pos[i])
  }
  
  # now we can do the area plot
  results$dfa <- data.frame(Return=x, Density=wpr[[vno]])
  results$dfb <- data.frame(Return=x, Density=wv[[vno]])
  results$dfc <- data.frame(Return=x, Density=wpo[[vno]])
  return (results)
}

er <- results$er
Sigma <- V
postSigma <- results$posteriorSigma
r <- plot_views(1, pir, er, Q, P, tau, Sigma, Omega, postSigma)
ggplot(r$dfa, aes(x=Return, y=Density)) + 
    geom_ribbon(aes(ymin=0, ymax=r$dfc[[2]],fill='blue')) +
    geom_ribbon(aes(ymin=0, ymax=r$dfb[[2]],fill='green')) +
    geom_ribbon(aes(ymin=0, ymax=r$dfa[[2]],fill='red')) +
    ggtitle('View 1 - Germany will outperform other European Markets by 5%') +
    ...
    scale_x_continuous(label = percent_formatter) +
    scale_fill_identity(name='Distribution',guide='legend',labels=c('Posterior','View','Prior'))
  
  #  This is for a line chart rather than the filled ribbon chart
  #  ggplot(df1, aes(x=Return, y=Density)) + 
  #    geom_line(aes(y=df3[[2]],color='blue')) +
  #    geom_line(aes(y=df2[[2]],color='green')) +
  #    geom_line(aes(y=df1[[2]],color='red')) +
  #    ylim(c(0,20)) +
  #    ggtitle('View 1 Adjusted - Investor twice as uncertain in view return') +
  #    theme(plot.title = element_text(size=20, face="bold") +
  #    theme(axis.ticks.y = element_blank(), axis.text.y = element_blank()) +
  #    scale_x_continuous(label = percent_formatter) +
  #    scale_color_manual(name='Distribution',values=c('red'='red','blue'='blue','green'='green'),labels=c('Posterior','View','Prior'))
  
r <- plot_views(2, pir, er, Q, P, tau, Sigma, Omega, postSigma)
ggplot(r$dfa, aes(x=Return, y=Density)) + 
  geom_ribbon(aes(ymin=0, ymax=r$dfc[[2]],fill='blue')) +
  geom_ribbon(aes(ymin=0, ymax=r$dfb[[2]],fill='green')) +
  geom_ribbon(aes(ymin=0, ymax=r$dfa[[2]],fill='red')) +
  ggtitle('View 2 - Canada will outperform the US by 4%') +
  theme(plot.title = element_text(size=20, face="bold")) +  
  theme(axis.ticks.y = element_blank(), axis.text.y = element_blank()) +
  theme(axis.title.x = element_text(size=20, face="bold")) + 
  theme(axis.title.y = element_text(size=20, face="bold")) + 
...
r <- plot_views(3, pir, er, Q, P, tau, Sigma, Omega, postSigma)
ggplot(r$dfa, aes(x=Return, y=Density)) + 
  geom_ribbon(aes(ymin=0, ymax=r$dfc[[2]],fill='blue')) +
  geom_ribbon(aes(ymin=0, ymax=r$dfb[[2]],fill='green')) +
  geom_ribbon(aes(ymin=0, ymax=r$dfa[[2]],fill='red')) +
  ggtitle('View 3 - Neutral view on Canada vs Japan') +
  theme(plot.title = element_text(size=20, face="bold")) +  
  theme(axis.ticks.y = element_blank(), axis.text.y = element_blank()) +
  theme(axis.title.x = element_text(size=20, face="bold")) + 
  theme(axis.title.y = element_text(size=20, face="bold")) + 
